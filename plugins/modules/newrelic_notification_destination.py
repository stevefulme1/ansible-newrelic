...
def main():
    ...
    if state == "present":
        existing = None
        if resource_id:
            existing = client.get("notification_destination", resource_id)
        elif module.params.get("name"):
            candidates = client.list("notification_destination", {{"name": module.params["name"]}})
            if candidates:
                existing = candidates[0]

        if existing:
            if module.check_mode:
                module.exit_json(changed=False, notification_destination=existing)
            result = client.update("notification_destination", resource_id or existing.get("id", ""), module.params)
