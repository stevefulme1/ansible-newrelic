...
def main():
    ...
    if state == "present":
        existing = None
        if resource_id:
            existing = client.get("log", resource_id)
        elif module.params.get("name"):
            candidates = client.list("log", {{"name": module.params["name"]}})
            if candidates:
                existing = candidates[0]

        if existing:
            if module.check_mode:
                module.exit_json(changed=False, log=existing)
            result = client.update("log", resource_id or existing.get("id", ""), module.params)
