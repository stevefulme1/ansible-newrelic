"""Unit tests for stevefulme1.newrelic.newrelic_workload module."""
...
MODULE_PATH = "ansible_collections.stevefulme1.newrelic.plugins.modules.newrelic_workload"
CLIENT_PATH = "ansible_collections.stevefulme1.newrelic.plugins.module_utils.api_client"


@pytest.fixture
def mock_api_client():
    """Mock API client for newrelic_workload."""
    client = MagicMock()
    client.get.return_value = None
    client.create.return_value = {"workload_id": "res-123", "name": "test-workload"}
    client.update.return_value = {"workload_id": "res-123", "name": "test-workload-updated"}
...
def existing_resource():
    """Return a dict representing an existing workload."""
    return {
        "workload_id": "res-123",
        "name": "test-workload",
        "state": "active",
...
class TestCreateWorkload:
    """Tests for creating a workload."""

    def test_create_returns_resource(self, mock_api_client):
        """Verify create returns resource dict with expected fields."""
        result = mock_api_client.create("workload", {"name": "test-workload"})
        assert result["workload_id"] == "res-123"
    ...
    def test_create_with_all_params(self, mock_api_client):
        ...
        mock_api_client.create("workload", params)
        mock_api_client.create.assert_called_once_with("workload", params)
...
class TestUpdateWorkload:
    ...
    def test_update_existing_resource(self, mock_api_client, existing_resource):
        ...
        result = mock_api_client.update("workload", "res-123", {"name": "updated"})
        assert result["name"] == "test-workload-updated"
    ...
    def test_update_nonexistent_raises(self, mock_api_client):
        ...
        with pytest.raises(Exception, match="404 Not Found"):
            mock_api_client.update("workload", "bad-id", {})


class TestDeleteWorkload:
    """Tests for deleting a workload."""

    def test_delete_existing(self, mock_api_client, existing_resource):
        """Verify delete calls API with correct ID."""
        mock_api_client.get.return_value = existing_resource
        mock_api_client.delete("workload", "res-123")
        mock_api_client.delete.assert_called_once_with("workload", "res-123")

    def test_delete_nonexistent_is_noop(self, mock_api_client):
        """Verify deleting absent resource reports no change."""
        mock_api_client.get.return_value = None
        result = mock_api_client.get("workload", "missing-id")
...
class TestGetWorkload:
    ...
    def test_get_api_timeout(self):
        ...
        with pytest.raises(TimeoutError):
            client.get("workload", "res-123")


class TestListWorkload:
    """Tests for listing workload resources."""

    def test_list_returns_all(self, mock_api_client):
        """Verify list returns all resources."""
        mock_api_client.list.return_value = [
            {"workload_id": "1", "name": "first"},
            {"workload_id": "2", "name": "second"},
        ]
        result = mock_api_client.list("workload")
        assert len(result) == 2
    ...
    def test_list_with_filter(self, mock_api_client):
        ...
        mock_api_client.list.return_value = [{"workload_id": "1", "name": "match"}]
        result = mock_api_client.list("workload", filters={"name": "match"})
        assert len(result) == 1


class TestIdempotencyWorkload:
    """Tests for idempotent behavior of workload."""
    ...
    def test_create_existing_is_idempotent(self, mock_api_client, existing_resource):
        ...
        mock_api_client.get.return_value = existing_resource
        current = mock_api_client.get("workload", "res-123")
    ...
    def test_delete_absent_is_idempotent(self, mock_api_client):
        ...
        mock_api_client.get.return_value = None
        exists = mock_api_client.get("workload", "missing") is not None
        assert exists is False


class TestErrorHandlingWorkload:
    """Tests for error handling in workload."""
