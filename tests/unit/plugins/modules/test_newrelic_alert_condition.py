"""Unit tests for stevefulme1.newrelic.newrelic_alert_condition module."""
...
MODULE_PATH = "ansible_collections.stevefulme1.newrelic.plugins.modules.newrelic_alert_condition"
CLIENT_PATH = "ansible_collections.stevefulme1.newrelic.plugins.module_utils.api_client"


@pytest.fixture
def mock_api_client():
    """Mock API client for newrelic_alert_condition."""
    client = MagicMock()
    client.get.return_value = None
    client.create.return_value = {"condition_id": "res-123", "name": "test-alert_condition"}
    client.update.return_value = {"condition_id": "res-123", "name": "test-alert_condition-updated"}
...
def existing_resource():
    """Return a dict representing an existing alert_condition."""
    return {
        "condition_id": "res-123",
        "name": "test-alert_condition",
        "state": "active",
    }


class TestCreateAlertCondition:
    """Tests for creating a alert_condition."""

    def test_create_returns_resource(self, mock_api_client):
        """Verify create returns resource dict with expected fields."""
        result = mock_api_client.create("alert_condition", {"name": "test-alert_condition"})
        assert result["condition_id"] == "res-123"
        assert result["name"] == "test-alert_condition"
        mock_api_client.create.assert_called_once()

    def test_create_with_all_params(self, mock_api_client):
        """Verify create passes all parameters to API."""
        params = {
            "name": "full-alert_condition",
            "description": "Full test",
            "tags": {"env": "test"},
        }
        mock_api_client.create("alert_condition", params)
        mock_api_client.create.assert_called_once_with("alert_condition", params)
    ...
    def test_create_api_error(self):
        ...
        with pytest.raises(Exception, match="409 Conflict"):
            client.create("alert_condition", {"name": "dup"})

    def test_create_check_mode_no_api_call(self, mock_api_client):
        """Verify check_mode skips actual API call."""
        check_mode = True
        if check_mode:
            result = {"changed": True, "alert_condition": {}}
        else:
            result = mock_api_client.create("alert_condition", {})
        assert result["changed"] is True
        mock_api_client.create.assert_not_called()


class TestUpdateAlertCondition:
    """Tests for updating a alert_condition."""
    ...
    def test_update_existing_resource(self, mock_api_client, existing_resource):
        ...
        result = mock_api_client.update("alert_condition", "res-123", {"name": "updated"})
        assert result["name"] == "test-alert_condition-updated"
    ...
    def test_update_nonexistent_raises(self, mock_api_client):
        ...
        with pytest.raises(Exception, match="404 Not Found"):
            mock_api_client.update("alert_condition", "bad-id", {})


class TestDeleteAlertCondition:
    """Tests for deleting a alert_condition."""

    def test_delete_existing(self, mock_api_client, existing_resource):
        """Verify delete calls API with correct ID."""
        mock_api_client.get.return_value = existing_resource
        mock_api_client.delete("alert_condition", "res-123")
        mock_api_client.delete.assert_called_once_with("alert_condition", "res-123")

    def test_delete_nonexistent_is_noop(self, mock_api_client):
        """Verify deleting absent resource reports no change."""
        mock_api_client.get.return_value = None
        result = mock_api_client.get("alert_condition", "missing-id")
    ...
    def test_delete_api_error(self):
        ...
        with pytest.raises(Exception, match="403 Forbidden"):
            client.delete("alert_condition", "res-123")


class TestGetAlertCondition:
    """Tests for getting a alert_condition."""

    def test_get_existing(self, mock_api_client, existing_resource):
        """Verify get returns resource when it exists."""
        mock_api_client.get.return_value = existing_resource
        result = mock_api_client.get("alert_condition", "res-123")
        assert result["condition_id"] == "res-123"

    def test_get_nonexistent(self, mock_api_client):
        """Verify get returns None for missing resource."""
        mock_api_client.get.return_value = None
        result = mock_api_client.get("alert_condition", "nonexistent")
        assert result is None
    ...
    def test_get_api_timeout(self):
        ...
        with pytest.raises(TimeoutError):
            client.get("alert_condition", "res-123")


class TestListAlertCondition:
    """Tests for listing alert_condition resources."""
    ...
    def test_list_returns_all(self, mock_api_client):
        ...
        result = mock_api_client.list("alert_condition")
        assert len(result) == 2
    ...
    def test_list_with_filter(self, mock_api_client):
        ...
        result = mock_api_client.list("alert_condition", filters={"name": "match"})
        assert len(result) == 1


class TestIdempotencyAlertCondition:
    """Tests for idempotent behavior of alert_condition."""
    ...
    def test_create_existing_is_idempotent(self, mock_api_client, existing_resource):
        ...
        mock_api_client.get.return_value = existing_resource
        current = mock_api_client.get("alert_condition", "res-123")
    ...
    def test_delete_absent_is_idempotent(self, mock_api_client):
        ...
        mock_api_client.get.return_value = None
        exists = mock_api_client.get("alert_condition", "missing") is not None
        assert exists is False


class TestErrorHandlingAlertCondition:
    """Tests for error handling in alert_condition."""
