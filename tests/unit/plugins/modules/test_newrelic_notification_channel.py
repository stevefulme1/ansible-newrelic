"""Unit tests for stevefulme1.newrelic.newrelic_notification_channel module."""
...
MODULE_PATH = "ansible_collections.stevefulme1.newrelic.plugins.modules.newrelic_notification_channel"
CLIENT_PATH = "ansible_collections.stevefulme1.newrelic.plugins.module_utils.api_client"


@pytest.fixture
def mock_api_client():
    """Mock API client for newrelic_notification_channel."""
    client = MagicMock()
    client.get.return_value = None
    client.create.return_value = {"channel_id": "res-123", "name": "test-notification_channel"}
    client.update.return_value = {"channel_id": "res-123", "name": "test-notification_channel-updated"}
...
def existing_resource():
    """Return a dict representing an existing notification_channel."""
    return {
        "channel_id": "res-123",
        "name": "test-notification_channel",
        "state": "active",
    }


class TestCreateNotificationChannel:
    """Tests for creating a notification_channel."""

    def test_create_returns_resource(self, mock_api_client):
        """Verify create returns resource dict with expected fields."""
        result = mock_api_client.create("notification_channel", {"name": "test-notification_channel"})
        assert result["channel_id"] == "res-123"
        assert result["name"] == "test-notification_channel"
        mock_api_client.create.assert_called_once()

    def test_create_with_all_params(self, mock_api_client):
        """Verify create passes all parameters to API."""
        params = {
            "name": "full-notification_channel",
            "description": "Full test",
            "tags": {"env": "test"},
        }
        mock_api_client.create("notification_channel", params)
        mock_api_client.create.assert_called_once_with("notification_channel", params)
...
class TestUpdateNotificationChannel:
    ...
    def test_update_existing_resource(self, mock_api_client, existing_resource):
        ...
        result = mock_api_client.update("notification_channel", "res-123", {"name": "updated"})
        assert result["name"] == "test-notification_channel-updated"
    ...
    def test_update_nonexistent_raises(self, mock_api_client):
        ...
        with pytest.raises(Exception, match="404 Not Found"):
            mock_api_client.update("notification_channel", "bad-id", {})


class TestDeleteNotificationChannel:
    """Tests for deleting a notification_channel."""

    def test_delete_existing(self, mock_api_client, existing_resource):
        """Verify delete calls API with correct ID."""
        mock_api_client.get.return_value = existing_resource
        mock_api_client.delete("notification_channel", "res-123")
        mock_api_client.delete.assert_called_once_with("notification_channel", "res-123")

    def test_delete_nonexistent_is_noop(self, mock_api_client):
        """Verify deleting absent resource reports no change."""
        mock_api_client.get.return_value = None
        result = mock_api_client.get("notification_channel", "missing-id")
...
class TestGetNotificationChannel:
    ...
    def test_get_api_timeout(self):
        ...
        with pytest.raises(TimeoutError):
            client.get("notification_channel", "res-123")


class TestListNotificationChannel:
    """Tests for listing notification_channel resources."""

    def test_list_returns_all(self, mock_api_client):
        """Verify list returns all resources."""
        mock_api_client.list.return_value = [
            {"channel_id": "1", "name": "first"},
            {"channel_id": "2", "name": "second"},
        ]
        result = mock_api_client.list("notification_channel")
        assert len(result) == 2
    ...
    def test_list_with_filter(self, mock_api_client):
        ...
        mock_api_client.list.return_value = [{"channel_id": "1", "name": "match"}]
        result = mock_api_client.list("notification_channel", filters={"name": "match"})
        assert len(result) == 1


class TestIdempotencyNotificationChannel:
    """Tests for idempotent behavior of notification_channel."""
    ...
    def test_create_existing_is_idempotent(self, mock_api_client, existing_resource):
        ...
        mock_api_client.get.return_value = existing_resource
        current = mock_api_client.get("notification_channel", "res-123")
    ...
    def test_delete_absent_is_idempotent(self, mock_api_client):
        ...
        mock_api_client.get.return_value = None
        exists = mock_api_client.get("notification_channel", "missing") is not None
        assert exists is False


class TestErrorHandlingNotificationChannel:
    """Tests for error handling in notification_channel."""
    ...
    def test_auth_failure(self):
        ...
        with pytest.raises(Exception, match="401 Unauthorized"):
            client.create("notification_channel", {})
    ...
    def test_rate_limit(self):
        ...
        with pytest.raises(Exception, match="429"):
            client.list("notification_channel")
