"""Unit tests for stevefulme1.newrelic.newrelic_entity_tag module."""
...
MODULE_PATH = "ansible_collections.stevefulme1.newrelic.plugins.modules.newrelic_entity_tag"
CLIENT_PATH = "ansible_collections.stevefulme1.newrelic.plugins.module_utils.api_client"


@pytest.fixture
def mock_api_client():
    """Mock API client for newrelic_entity_tag."""
    client = MagicMock()
    client.get.return_value = None
    client.create.return_value = {"entity_guid": "res-123", "tag_key": "test-entity_tag"}
    client.update.return_value = {"entity_guid": "res-123", "tag_key": "test-entity_tag-updated"}
...
def existing_resource():
    """Return a dict representing an existing entity_tag."""
    return {
        "entity_guid": "res-123",
        "tag_key": "test-entity_tag",
        "state": "active",
    }


class TestCreateEntityTag:
    """Tests for creating a entity_tag."""

    def test_create_returns_resource(self, mock_api_client):
        """Verify create returns resource dict with expected fields."""
        result = mock_api_client.create("entity_tag", {"tag_key": "test-entity_tag"})
        assert result["entity_guid"] == "res-123"
        assert result["tag_key"] == "test-entity_tag"
        mock_api_client.create.assert_called_once()

    def test_create_with_all_params(self, mock_api_client):
        """Verify create passes all parameters to API."""
        params = {
            "tag_key": "full-entity_tag",
            "description": "Full test",
            "tags": {"env": "test"},
        }
        mock_api_client.create("entity_tag", params)
        mock_api_client.create.assert_called_once_with("entity_tag", params)
...
class TestUpdateEntityTag:
    ...
    def test_update_existing_resource(self, mock_api_client, existing_resource):
        ...
        result = mock_api_client.update("entity_tag", "res-123", {"tag_key": "updated"})
        assert result["tag_key"] == "test-entity_tag-updated"
    ...
    def test_update_idempotent_no_change(self, mock_api_client, existing_resource):
        ...
        desired = {"tag_key": existing_resource["tag_key"]}
        current = {"tag_key": existing_resource["tag_key"]}
    ...
    def test_update_nonexistent_raises(self, mock_api_client):
        ...
        with pytest.raises(Exception, match="404 Not Found"):
            mock_api_client.update("entity_tag", "bad-id", {})


class TestDeleteEntityTag:
    """Tests for deleting a entity_tag."""

    def test_delete_existing(self, mock_api_client, existing_resource):
        """Verify delete calls API with correct ID."""
        mock_api_client.get.return_value = existing_resource
        mock_api_client.delete("entity_tag", "res-123")
        mock_api_client.delete.assert_called_once_with("entity_tag", "res-123")

    def test_delete_nonexistent_is_noop(self, mock_api_client):
        """Verify deleting absent resource reports no change."""
        mock_api_client.get.return_value = None
        result = mock_api_client.get("entity_tag", "missing-id")
...
class TestGetEntityTag:
    ...
    def test_get_api_timeout(self):
        ...
        with pytest.raises(TimeoutError):
            client.get("entity_tag", "res-123")


class TestListEntityTag:
    """Tests for listing entity_tag resources."""

    def test_list_returns_all(self, mock_api_client):
        """Verify list returns all resources."""
        mock_api_client.list.return_value = [
            {"entity_guid": "1", "tag_key": "first"},
            {"entity_guid": "2", "tag_key": "second"},
        ]
        result = mock_api_client.list("entity_tag")
        assert len(result) == 2
    ...
    def test_list_with_filter(self, mock_api_client):
        ...
        mock_api_client.list.return_value = [{"entity_guid": "1", "tag_key": "match"}]
        result = mock_api_client.list("entity_tag", filters={"tag_key": "match"})
        assert len(result) == 1


class TestIdempotencyEntityTag:
    """Tests for idempotent behavior of entity_tag."""
    ...
    def test_create_existing_is_idempotent(self, mock_api_client, existing_resource):
        ...
        mock_api_client.get.return_value = existing_resource
        current = mock_api_client.get("entity_tag", "res-123")
        desired_params = {"tag_key": current["tag_key"]}
        # If resource exists and matches desired state, no change
        changed = desired_params["tag_key"] != current["tag_key"]
        assert changed is False

    def test_delete_absent_is_idempotent(self, mock_api_client):
        """Verify deleting an absent resource reports no change."""
        mock_api_client.get.return_value = None
        exists = mock_api_client.get("entity_tag", "missing") is not None
        assert exists is False


class TestErrorHandlingEntityTag:
    """Tests for error handling in entity_tag."""
